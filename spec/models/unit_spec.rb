...
RSpec.describe Unit do
  describe 'table' do
    ...
    it { is_expected.to have_db_column(:user_daily_page_limit).of_type(:integer).with_options(null: false, default: 30) }
  ...
  describe 'validations' do
    ...
    it { is_expected.to validate_presence_of(:user_daily_page_limit) }
  ...
  describe '#total_pages_processed' do
    ...
    it 'sums the page_count of completed or processing jobs for API and GUI users in the unit' do
      api_user1 = create(:api_user, unit: unit)
      api_user2 = create(:api_user, unit: unit)
      gui_user = create(:gui_user, unit: unit)

      create(:pdf_job, status: 'completed', owner: api_user1, page_count: 3)
      create(:pdf_job, status: 'processing', owner: api_user1, page_count: 2)
      create(:pdf_job, status: 'completed', owner: api_user2, page_count: 5)
      create(:pdf_job, status: 'processing', owner: gui_user, page_count: 7)
      create(:pdf_job, status: 'failed', owner: gui_user, page_count: 7)

      other_unit = create(:unit)
      other_api_user = create(:api_user, unit: other_unit)
      other_gui_user = create(:gui_user, unit: other_unit)
      create(:pdf_job, status: 'completed', owner: other_api_user, page_count: 100)
      create(:pdf_job, status: 'completed', owner: other_gui_user, page_count: 200)
