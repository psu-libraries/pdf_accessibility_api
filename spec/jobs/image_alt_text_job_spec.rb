...
RSpec.describe ImageAltTextJob do
  ...
  describe '#perform' do
    context 'when the job is called with job uuid and ' do
      before do
        described_class.perform_now(job.uuid, file_upload.to_json)
      end

      it 'calls the Alt Text gem' do
        described_class.perform_now(job.uuid, file_upload.to_json)
        expect(alt_text_gem).to have_received(:process_image).with('abjlkjf', File.read('prompt.txt'), ENV['LLM_MODEL'])
      end
    ...
    context 'when alt-text is not produced before the timeout is exceeded' do
      before { alt_text_response = nil }
      it 'updates the status and metadata of the given image job record' do
        described_class.perform_now(job.uuid, file_upload.to_json, output_polling_timeout: 1)
        reloaded_job = job.reload
        expect(reloaded_job.status).to eq 'completed'
        expect(reloaded_job.finished_at).to be_within(1.minute).of(Time.zone.now)
        expect(reloaded_job.alt_text).to eq('Generated Alt-text')
      end
    end
